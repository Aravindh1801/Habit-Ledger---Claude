...
const LAST_BACKUP_KEY = "ledger-last-backup-v1";
...
function App() {
  ...
  const [lastBackup, setLastBackup] = React.useState(loadLastBackup);
  ...
  function backupToFile() {
    ...
    a.href = url;
    a.download = `ledger-backup-${dateKey(new Date())}.json`;
  ...
  function handleRestoreFile(ev) {
    ...
    reader.onload = () => {
      try {
        ...
        const restored = Array.isArray(parsed) ? parsed : Array.isArray(parsed.habits) ? parsed.habits : null;
        if (!restored) throw new Error("bad format");
        const count = restored.length;
        if (!window.confirm(`Replace current data with this backup (${count} habit${count === 1 ? "" : "s"})? This can't be undone.`)) {
          return;
        }
        setHabits(restored);
      } catch {
        setErr("That file doesn't look like a Ledger backup.");
        setTimeout(() => setErr(""), 3000);
      }
    };
  ...
  return e(
    ...
    e(
      ...
      e(
        "div",
        { style: { marginTop: 22, paddingTop: 16, borderTop: `1px solid ${ink.border}` } },
        e(
          "div",
        ...
        e(
          "div",
          { style: { fontSize: 11, color: ink.textDim, marginTop: 8, lineHeight: 1.5 } },
          lastBackup
            ? `Last backup: ${new Date(lastBackup).toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" })}`
            : "No backups yet — your data currently only lives in this browser."
        ),
        e("input", {
          type: "file",
